...
export default function TikTokCard({ video, userName, isLiked, onAnalyzeClick, onLikeClick }: TikTokCardProps) {
    ...
    return (
        ...
        >
            ...
            {/* --- VÙNG HIỂN THỊ TAG niche) --- */}
            {video.niche && (
                <div className="px-4 py-2 bg-gradient-to-r from-purple-100 to-blue-100 text-purple-800 text-xs font-semibold rounded-tl-lg rounded-tr-lg">
                    {video.niche}
                </div>
            )}

            {/* --- VÙNG MÔ TẢ VIDEO (Giữ nguyên) --- */}
